...
| --- | ---: | --- | --- |
| NIM | 16 | `cust-5HhTkEYiKpPvLAnRXJS62w` | Running on `ubuntu-local-dev` |
| NIM | 32 | `cust-MPT4gXsS5wWpLwcz2J3Dhx` | Running on `ubuntu-local-dev` |
| NeMo Microservices | 16 | `cust-SUFrUtLpBiduPqnma44F7t` | Queued for `ubuntu-local-dev` |
| NeMo Microservices | 32 | `cust-8c4VynVebA9CCjwH5p1kHE` | Queued for `ubuntu-local-dev` |
...
2. Pointing the same global selector at `ubuntu2` also moved GPU trainers to
...
3. Correct split placement was produced by pausing NeMo operator
   reconciliation, submitting each pair with handlers rendered for `ubuntu2`,
   patching only
   `spec.trainingWorkload.trainerOverrides.spec.nodeSelector` to
   `ubuntu-local-dev`, and then resuming the operator.
...
- NIM handlers completed once on `ubuntu2`.
...
- NIM r=16 uses approximately 65.8 GiB GPU memory; r=32 uses approximately
  69.9 GiB.
- Both NeMo handlers completed once on `ubuntu2`.
- Both NeMo trainer pods have the `ubuntu-local-dev` selector and are pending
