#!/usr/bin/env python3
"""Launch Curator 26.04 as its packaged user and securely inject the API key."""
...
def mounted_path(path: Path) -> str:
    relative = experiment_path(path).relative_to(EXPERIMENT_ROOT)
...
def main() -> int:
    ...
    completed = subprocess.run(run_command, env=environment, check=False)
    if output_dir.exists():
        subprocess.run(
            [
                "docker",
