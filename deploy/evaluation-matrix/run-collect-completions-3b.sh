#!/usr/bin/env bash
# Run durable Stage 3 completion collection for Llama 3.2 3B targets.
#
# Set RUN_ID to align this job with another active completion matrix:
#   RUN_ID=20260529T142723Z deploy/evaluation-matrix/run-collect-completions-3b.sh
set -euo pipefail

ROOT="$(cd "$(dirname "$0")/../.." && pwd)"
NS="${NS:-runai-rag}"
RUN_ID="${RUN_ID:-$(date -u +%Y%m%dT%H%M%SZ)}"
JOB_MANIFEST="$ROOT/deploy/evaluation-matrix/collect-completions-3b-job.yaml"
...
echo "==> creating 3B completion collection Job from $JOB_MANIFEST with RUN_ID=$RUN_ID"
kubectl create -f "$TMP_MANIFEST"

echo "==> recent 3B completion collector jobs"
kubectl get jobs -n "$NS" \
  -l app.kubernetes.io/name=completion-collector,app.kubernetes.io/component=llama-3.2-3b \
