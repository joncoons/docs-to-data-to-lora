"""Tests for direct LLM single-axis judging over saved completions."""

import json
from pathlib import Path

from scripts.eval.direct_llm_singleaxis_impl import (
    JudgeConfig,
    build_judge_user,
    coerce_scores,
...
def test_build_judge_user_splits_question_and_uses_response_without_context():
...
def test_output_dir_preserves_completion_layout(tmp_path):
    ...
    assert output_dir_for(
        ...
        output_root=tmp_path / "singleaxis-llm",
        eval_run_id="evalrun",
    ) == (
        tmp_path
        / "singleaxis-llm"
