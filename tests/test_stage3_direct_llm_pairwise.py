"""Tests for direct LLM pairwise judging over saved completions."""

import json

from scripts.eval.direct_llm_pairwise_impl import (
    build_judge_user,
    build_pair_rows,
...
def test_build_judge_user_splits_question_and_swaps_answers_without_context():
    left = {
    ...
    assert "Immutable reference answer:\nFact one is true." in user
...
def test_pair_output_dir_uses_dataset_and_labels(tmp_path):
    ...
    out_dir = pair_output_dir(
        ...
        tmp_path / "pairwise-llm",
        "run1",
    )

    assert out_dir == tmp_path / "pairwise-llm" / "nim_curated" / "1b-r16__vs__1b-r32" / "run1"
