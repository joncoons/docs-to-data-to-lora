#!/usr/bin/env python3
"""Run no-RAG direct LLM single-axis judging over saved completions.
...
DEFAULT_OUTPUT_ROOT = Path(
    os.getenv("SINGLEAXIS_OUTPUT_ROOT", "<EVAL_ROOT>/singleaxis-llm")
)
DEFAULT_REPO_SUMMARY_DIR = Path(os.getenv("SINGLEAXIS_REPO_SUMMARY_DIR", "artifacts/evaluation/singleaxis-summaries"))
...
async def judge_one(
    ...
    for attempt in range(config.max_attempts):
        ...
        try:
            ...
            return {
                "schema_version": "direct-llm-singleaxis-row/v1",
        ...
        async def run_one(item: tuple[int, dict[str, Any]]) -> tuple[bool, dict[str, Any]]:
            ...
            async with semaphore:
                try:
                    ...
                    return False, {
                        "schema_version": "direct-llm-singleaxis-error/v1",
                        "source_row_index": source_row_index(row),
                        "source_line_number": source_line_number(row),
                        "row_ordinal": ordinal,
                        "dataset_slug": row.get("dataset_slug"),
...
def run_one_responses_file(
    ...
    *,
    ...
    manifest = {
        "schema_version": "direct-llm-singleaxis-run/v1",
        "eval_run_id": eval_run_id,
