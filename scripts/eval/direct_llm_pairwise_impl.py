#!/usr/bin/env python3
"""Run direct LLM pairwise judging over saved completion files.
...
DEFAULT_OUTPUT_ROOT = Path(
    os.getenv("PAIRWISE_OUTPUT_ROOT", "<EVAL_ROOT>/pairwise-llm")
)
DEFAULT_REPO_SUMMARY_DIR = Path(os.getenv("PAIRWISE_REPO_SUMMARY_DIR", "artifacts/evaluation/pairwise-summaries"))
...
async def judge_pair_row(
    ...
    *,
    ...
    return {
        "schema_version": "direct-llm-pairwise-row/v1",
        ...
        async def run_one(item: tuple[int, dict[str, Any], dict[str, Any]]) -> tuple[bool, dict[str, Any]]:
            ...
            async with semaphore:
                try:
                    ...
                    return False, {
                        "schema_version": "direct-llm-pairwise-error/v1",
                        "source_row_index": source_row_index(left),
                        "left_source_line_number": source_line_number(left),
                        "right_source_line_number": source_line_number(right),
                        "row_ordinal": row_ordinal,
...
def run_one_pair(
    ...
    *,
    ...
    manifest = {
        "schema_version": "direct-llm-pairwise-run/v1",
