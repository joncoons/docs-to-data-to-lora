#!/usr/bin/env python3
"""Resumable Kimi seed generation for grounded Data Designer augmentation."""
...
def run(config: SeedConfig, *, key_file: Path | None, fallback_on_error: bool, progress_interval: int) -> dict[str, Any]:
    if config.mode != "kimi":
        raise RuntimeError("resumable runner is intended for --mode kimi")
    if not config.allow_external_kimi:
        raise RuntimeError("--allow-external-kimi is required")
    ...
    failures_path = data_designer_dir / "kimi_seed_failures.jsonl"
    progress_path = data_designer_dir / "kimi_seed_progress.json"
    ...
    def write_progress(status: str) -> None:
        ...
        write_json(progress_path, {
            "schema_version": "resumable-kimi-seeds.v1",
            "updated_at": utc_now(),
            "status": status,
    ...
    def build_one(idx: int, candidate: dict[str, Any]) -> tuple[int, dict[str, Any], dict[str, Any] | None]:
        try:
            ...
            seed["seed_author"] = "heuristic_fallback_after_kimi_error"
            seed["kimi"]["failure"] = failure
    ...
    if failures_path.exists():
        artifacts.append(artifact_manifest(failures_path, config.output_dir, "kimi_seed_failures"))
...
def parse_args(argv: list[str] | None = None) -> argparse.Namespace:
    ...
    ap.add_argument("--judge-api-url", default="https://inference-api.nvidia.com/v1/chat/completions")
    ap.add_argument("--judge-model", default="nvidia/moonshotai/kimi-k2.6")
    ap.add_argument("--judge-api-key-env", default="NV_KIMI_KEY")
